...
@app.route( '/') # to login or not to login, that is the question
def home():
    if isLoggedIn == True: # if already in session (already logged in)
        return redirect( '/welcome')
    #session.pop( 'username')
    #session.pop( 'password')
    session.pop
    if session[ 'username'] != "davidmholmes":
        return redirect('/unError')
    if session[ 'password'] != "apcs": 
